...
public class ExpansionPanelsCode extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_expansion_panels_code);
    ...
    private void initToolbar() {
        ...
        txtTitle.setText("Expansion Panels");
    ...
    public String codeCollectorXML(MenuItem item) {
        if (item.getTitle() == "Basic") {
            try {
                InputStream is = getAssets().open("expansionpanelsBasicXML.xml");
            ...
            try {
                InputStream is = getAssets().open("expansionpanelsInvoiceXML.xml");
                int size = is.available();
                byte[] buffer = new byte[size];
                is.read(buffer);
                is.close();
            ...
            try {
                InputStream is = getAssets().open("expansionpanelsTicketXML.xml");
                int size = is.available();
                byte[] buffer = new byte[size];
                is.read(buffer);
                is.close();
    ...
    public String codeCollectorJAVA(MenuItem item) {
        if (item.getTitle() == "Basic") {
            try {
                InputStream is = getAssets().open("expansionpanelsBasicJAVA.java");
                int size = is.available();
                byte[] buffer = new byte[size];
                is.read(buffer);
                is.close();
            ...
            try {
                InputStream is = getAssets().open("expansionpanelsInvoiceJAVA.java");
                int size = is.available();
                byte[] buffer = new byte[size];
                is.read(buffer);
                is.close();
            ...
            try {
                InputStream is = getAssets().open("expansionpanelsTicketJAVA.java");
                int size = is.available();
                byte[] buffer = new byte[size];
                is.read(buffer);
                is.close();
